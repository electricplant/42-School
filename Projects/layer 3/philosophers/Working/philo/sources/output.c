...
static void	print_stats_debug(t_philo *philo, t_status status)
{
	if (status == DIED)
...
void	print_stats(t_philo *philo, bool reaper_report, t_status status)
{
	pthread_mutex_lock(&philo->table->write_lock);
	if (sim_done(philo->table) == true && reaper_report == false)
	{
	...
	{
		print_stats_debug(philo, status);
		pthread_mutex_unlock(&philo->table->write_lock);
		return ;
	}
	if (status == DIED)
